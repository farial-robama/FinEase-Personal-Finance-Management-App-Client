...
const UpdateTransaction = ({transaction, onClose}) => {
    ...
    if (!transaction) return null;
      ...
      const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const res = await fetch(`http://localhost:5000/transactions/${transaction._id}`, {
                method: "PUT",
...
<div className="max-w-md mx-auto text-center bg-[#8FABD4] rounded-md p-3">
      ...
      <div className="card-body">
        ...
        <div className='flex gap-3 justify-end mt-3'>
            <button type="submit" className="btn btn-active bg-[#C2E2FA] text-[#39505a] cursor-pointer">
          Update
        </button>
        <button onClick={() => onClose(false)} className="btn btn-active bg-[#C2E2FA] text-[#39505a] cursor-pointer"> Cancel
  </button>
        </div>
